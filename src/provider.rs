...
use crate::{
    core::{
        HealthCheckRequest, HostData, Invocation, InvocationResponse,
        LinkDefinition,
    },
    deserialize,
...
impl ProviderConnection {
        ...
        P: Provider + Clone,
    {
        ...
        handles.push(self.subscribe_rpc(
            provider.clone(),
            shutdown_tx.subscribe(),
            lattice,
        ).await?);
        handles.push(self.subscribe_link_put(provider.clone(), shutdown_tx.subscribe()).await?);
        handles.push(self.subscribe_link_del(provider.clone(), shutdown_tx.subscribe()).await?);
        handles.push(self.subscribe_shutdown(provider.clone(), shutdown_tx.clone()).await?);
        handles.push(self.subscribe_health(provider, shutdown_tx.subscribe()).await?);
    ...
    async fn subscribe_link_put<P>(&self, provider: P, mut quit: QuitSignal) -> ProviderResult<JoinHandle<()>>
    where
        P: Provider + Clone,
    {
    ...
    async fn subscribe_link_del<P>(&self, provider: P, mut quit: QuitSignal) -> ProviderResult<JoinHandle<()>>
    where
        P: Provider + Clone,
    {
        // Link Delete
    ...
    async fn subscribe_health<P>(&self, provider: P, mut quit: QuitSignal) -> ProviderResult<JoinHandle<()>>
    where
        P: Provider,
    {
        let topic = format!(
    ...
    async fn validate_provider_invocation(
        ...
        if inv.target.public_key != self.host_data.provider_key {
            return Err(ValidationError::InvalidTarget(inv.target.public_key.clone(), self.host_data.provider_key.clone()));
        }
